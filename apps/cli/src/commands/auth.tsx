...
const DRAGON_WEB_URL =
  process.env.DRAGON_WEB_URL || "http://localhost:3000";
...
export function AuthCommand({ apiKey: providedApiKey }: AuthCommandProps) {
  ...
  useEffect(() => {
    ...
    server.listen(AUTH_PORT, () => {
      ...
      openBrowser(`${DRAGON_WEB_URL}/cli/auth`);
    });
    ...
    <Box flexDirection="column" paddingY={1}>
      {status === "success" ? (
        ...
        <>
          ...
          <Box marginTop={1}>
            <Text>
              Visit: <Text color="blue">{DRAGON_WEB_URL}/cli/auth</Text>
            </Text>
          </Box>
