...
export function useSaveApiOverrideMutation() {
  const queryClient = useQueryClient();
  return useServerActionMutation({
...
export function useSaveKimiConfigTomlMutation() {
  const queryClient = useQueryClient();
  return useServerActionMutation({
    mutationFn: saveKimiConfigToml,
    onSuccess: () => {
      toast.success("Kimi credentials saved");
      queryClient.invalidateQueries({
        queryKey: credentialsQueryKeys.list(),
      });
    },
