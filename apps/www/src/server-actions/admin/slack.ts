...
import { getSlackAccountAndInstallationForWorkspace } from "@dragon/shared/model/slack";
import { decryptValue } from "@dragon/utils/encryption";
...
> {
  ...
  if (!slackInstallation) {
    return {
      success: false,
      error: `No Slack installation found for workspace: ${workspaceDomain}. Make sure the workspace is connected to Dragon.`,
    };
  }
  if (!slackAccount) {
    return {
      success: false,
      error: `No Slack account found for workspace: ${workspaceDomain}. Make sure the workspace is connected to Dragon.`,
    };
  }
