...
import { ClaudeMessage } from "@dragon/daemon/shared";
import {
  DBMessage,
  DBSystemMessage,
  DBUserMessage,
...
import { extendSandboxLife } from "@dragon/sandbox";
...
export async function handleDaemonEvent({
  ...
  if (isError) {
    if (isPromptTooLong) {
      ...
      threadChatUpdates.errorMessage = "agent-generic-error";
      // Provide a generic error message when no specific error info is available
      threadChatUpdates.errorMessageInfo =
        "An unexpected error occurred. Please try again or start a new task.";
    }
    getPostHogServer().capture({
      distinctId: userId,
      event: "thread_error",
