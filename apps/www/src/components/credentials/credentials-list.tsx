...
// Helper to get display name for API override providers
function getApiOverrideDisplayName(provider: string): string {
  switch (provider) {
    case "kimi":
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex flex-row items-center gap-2">
            <span className="text-sm font-medium truncate">
              {/* Show API override provider name for opencode credentials */}
              {credential.agent === "opencode" &&
              credential.metadata &&
              typeof credential.metadata === "object" &&
              "type" in credential.metadata &&
              credential.metadata.type === "claude-api-override"
