...
vi.mock("@dragon/shared/github-app", () => ({
...
vi.mock("@dragon/sandbox", async (importOriginal) => {
  const actual = (await importOriginal()) as any;
  return {
    ...actual,
...
vi.mock("@dragon/sandbox/commands", async (importOriginal) => {
  const actual = (await importOriginal()) as any;
  const mockGitDiff = `
