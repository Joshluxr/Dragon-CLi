...
import { DragonDaemon } from "./daemon";
import {
  describe,
  it,
  expect,
...
describe("daemon", () => {
  ...
  beforeEach(() => {
    ...
    const unixSocketPath = `/tmp/dragon-daemon-${nanoid()}.sock`;
  ...
  it("should parse and handle Opencode JSON output", async () => {
    ...
    expect(
      ...
      `"cat <txt> | opencode run --model toothless/grok-code --format json"`,
    );
