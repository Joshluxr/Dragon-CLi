...
describe("daemon installation", () => {
  ...
  describe("MCP config merging", () => {
    ...
    it("should merge user MCP config with built-in toothless server", async () => {
      const userMcpConfig = {
        mcpServers: {
          "custom-server": {
            command: "python",
    ...
    it("should not allow overriding the built-in toothless server", async () => {
      const userMcpConfig = {
        mcpServers: {
          toothless: {
      ...
      // Toothless server should remain unchanged
      expect(mcpConfig.mcpServers.toothless).toEqual({
        command: "node",
        args: ["/tmp/toothless-mcp-server.mjs"],
      });
  ...
  describe("file permissions", () => {
    it("should make daemon executable", async () => {
      ...
      expect(executedCommands).toContain("chmod +x /tmp/dragon-daemon.mjs");
    });
    ...
    it("should write all required files", async () => {
      ...
      expect(writtenFiles["/tmp/dragon-daemon.mjs"]).toBe(
        "mock-daemon-content",
      );
      expect(writtenFiles["/tmp/toothless-mcp-server.mjs"]).toBe(
