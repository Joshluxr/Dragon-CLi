...
const REPO_DIR = "repo";
const BASE_IMAGE =
  process.env.DOCKER_SANDBOX_IMAGE || "ghcr.io/terragon-labs/containers-test";
...
export class DockerProvider implements ISandboxProvider {
  ...
  async extendLife(sandboxId: string): Promise<void> {
    // Extend the sandbox's life by running a simple command
    // This keeps the container active and resets any hibernation timers
    try {
      execSync(`docker exec ${sandboxId} echo 'heartbeat'`, {
        encoding: "utf8",
        stdio: "ignore",
      });
      console.log(`[docker] Extended life for sandbox ${sandboxId}`);
    } catch (error) {
      console.warn(`[docker] Failed to extend life for ${sandboxId}:`, error);
      // Don't throw - the container might be paused, in which case
      // we should try to unpause it first
      try {
        execSync(`docker unpause ${sandboxId}`, { stdio: "ignore" });
        execSync(`docker exec ${sandboxId} echo 'heartbeat'`, {
          encoding: "utf8",
        ...
        console.log(
          `[docker] Unpaused and extended life for sandbox ${sandboxId}`,
        );
      } catch {
        // Container might not exist or be in an unrecoverable state
        throw new Error(`Failed to extend life for sandbox ${sandboxId}`);
      }
    }
  }
  ...
  /**
   * Cleanup utility function to remove all Dragon containers
   * Useful for complete system cleanup
   */
  static async cleanupAllContainers(): Promise<void> {
    try {
      // Get all containers with any dragon prefix
      ...
      console.warn("Failed to cleanup Dragon containers:", error);
    }
  }
}
