...
export const DAEMON_FILE_PATH = "/tmp/dragon-daemon.mjs";
export const MCP_SERVER_FILE_PATH = "/tmp/toothless-mcp-server.mjs";
export const MCP_SERVER_JSON_FILE_PATH = "/tmp/mcp-server.json";
export const DAEMON_LOG_FILE_PATH = "/tmp/dragon-daemon.log";
    ...
    ].join(" "),
    {
      env: getEnv({
        ...
        overrides: {
          ...
          DRAGON_FEATURE_FLAGS: JSON.stringify(featureFlags),
        },
      }),
      onOutput: (data) => {
        console.log(data);
...
export async function installDaemon({
  ...
  // Merge user MCP config with built-in toothless server (shared logic with Codex)
...
export async function sendMessage({
  ...
  const filePath = `/tmp/dragon-msg-${Date.now()}.json`;
