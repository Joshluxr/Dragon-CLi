...
import { LEGACY_THREAD_CHAT_ID } from "@dragon/shared/utils/thread-utils";
import {
  Thread,
  ThreadInsert,
  ThreadInsertRaw,
...
import { BroadcastMessageThreadData } from "@dragon/types/broadcast";
...
/**
 * Status values for rate-limited queues (sandbox creation and agent rate limits)
 */
const RATE_LIMITED_QUEUE_STATUSES = [
  "queued-sandbox-creation-rate-limit",
  "queued-agent-rate-limit",
] as const;

/**
 * Status value for concurrency-limited queue
 */
const CONCURRENCY_QUEUE_STATUS = "queued-tasks-concurrency" as const;

/**
 * Build conditions for querying threads/threadChats that are eligible for dequeuing.
 * Handles agent rate limit (with reattemptQueueAt check), sandbox rate limit, and concurrency.
 */
function buildQueuedStatusConditions<
  T extends typeof schema.thread | typeof schema.threadChat,
>(
  table: T,
  options: {
    includeSandboxRateLimit: boolean;
    includeConcurrency: boolean;
  },
) {
  const now = new Date();
  const conditions = [
    // Agent rate limit - only if reattemptQueueAt has passed
    and(
      eq(table.status, "queued-agent-rate-limit"),
      lte(table.reattemptQueueAt, now),
    ),
  ];

  if (options.includeSandboxRateLimit) {
    conditions.push(eq(table.status, "queued-sandbox-creation-rate-limit"));
  }

  if (options.includeConcurrency) {
    conditions.push(eq(table.status, CONCURRENCY_QUEUE_STATUS));
  }
...
> {
  ...
  const query = db
    .select({
      ...
      agentMode: schema.thread.agentMode,
      executionPlanId: schema.thread.executionPlanId,
      autonomousMode: schema.thread.autonomousMode,
      autonomousExecutionId: schema.thread.autonomousExecutionId,
...
export async function getThread({
  ...
  return {
    ...
    executionPlanId: thread.executionPlanId,
    autonomousMode: thread.autonomousMode,
    autonomousExecutionId: thread.autonomousExecutionId,
...
/**
 * Resolve thread chats for a thread, handling both legacy (version 0) and modern formats.
 *
 * For legacy threads (version 0), the thread data itself contains the chat data,
 * so we create a synthetic ThreadChatInfoFull from the thread.
 *
 * For modern threads (version > 0), we use the actual threadChats relationship.
 *
 * @param thread - The thread to resolve chats for
 * @param chats - The thread chats from the relationship (may be empty for legacy threads)
 * @returns Array of thread chat info objects
 */
...
/**
 * Create a ThreadChatInfoFull object from a legacy thread (version 0).
 *
 * Legacy threads store chat data directly on the thread table. This function
 * maps that data to the ThreadChatInfoFull format using a special ID constant
 * (LEGACY_THREAD_CHAT_ID) to distinguish it from actual threadChat records.
 *
 * This maintains backwards compatibility during the migration period.
 *
 * @param thread - The legacy thread to convert
 * @returns A synthetic ThreadChatInfoFull object
 */
function createLegacyThreadChatFull(
  ...
  return {
    ...
    agentMode: thread.agentMode ?? "auto",
    executionPlanId: thread.executionPlanId ?? null,
    autonomousMode: thread.autonomousMode ?? false,
    autonomousExecutionId: thread.autonomousExecutionId ?? null,
  };
}
...
export async function getEligibleQueuedThreadChats({
  ...
  return await db.transaction(async (tx) => {
    // Build status conditions using the helper
    const conditionOptions = {
      includeSandboxRateLimit: !sandboxCreationRateLimitReached,
      includeConcurrency: !concurrencyLimitReached,
    };
    const threadStatusConditions = buildQueuedStatusConditions(
      schema.thread,
      conditionOptions,
    );
    const threadChatStatusConditions = buildQueuedStatusConditions(
      schema.threadChat,
      conditionOptions,
    );
...
export async function getUserIdsWithThreadsReadyToProcess({ db }: { db: DB }) {
  ...
  const [threads, threadChats] = await Promise.all([
    db
      ...
      .where(
        and(
          inArray(schema.thread.status, RATE_LIMITED_QUEUE_STATUSES),
          or(
    ...
    db
      ...
      .where(
        and(
          inArray(schema.threadChat.status, RATE_LIMITED_QUEUE_STATUSES),
          or(
  ...
  const [usersWithQueuedThreads, usersWithQueuedThreadChats] =
    await Promise.all([
      db
        ...
        .from(schema.thread)
        .where(eq(schema.thread.status, CONCURRENCY_QUEUE_STATUS)),
      db
        .selectDistinct({
          userId: schema.threadChat.userId,
        })
        .from(schema.threadChat)
        .where(eq(schema.threadChat.status, CONCURRENCY_QUEUE_STATUS)),
    ]);
  if (
...
/**
 * Update reattemptQueueAt for all rate-limited threads/threadChats belonging to a user.
 * This is used when we know the rate limit reset time and want to avoid unnecessary
 * retries before then.
 */
export async function updateReattemptQueueAtForUser({
  db,
  userId,
  reattemptQueueAt,
}: {
  db: DB;
  userId: string;
  reattemptQueueAt: Date;
}): Promise<{ updatedThreads: number; updatedThreadChats: number }> {
  const rateLimitStatuses = [
    "queued-sandbox-creation-rate-limit",
    "queued-agent-rate-limit",
  ] as const;

  const [threadResult, threadChatResult] = await Promise.all([
    db
      .update(schema.thread)
      .set({ reattemptQueueAt })
      .where(
        and(
          eq(schema.thread.userId, userId),
          inArray(schema.thread.status, rateLimitStatuses),
          or(
            isNull(schema.thread.reattemptQueueAt),
    ...
    db
      ...
      .where(
        and(
          eq(schema.threadChat.userId, userId),
          inArray(schema.threadChat.status, rateLimitStatuses),
          or(
            isNull(schema.threadChat.reattemptQueueAt),
  ...
  return {
    updatedThreads: threadResult.length,
    updatedThreadChats: threadChatResult.length,
  };
}
