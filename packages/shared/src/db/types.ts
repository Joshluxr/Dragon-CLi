...
// API Override metadata for Claude-compatible APIs (Kimi, GLM, etc.)
export type ClaudeApiOverrideProvider = "kimi" | "glm";

export type ClaudeApiOverrideMetadata = {
  type: "claude-api-override";
  provider: ClaudeApiOverrideProvider;
  baseUrl: string;
  // Optional for type compatibility with other metadata types that access accountId
  accountId?: string;
};

// Kimi subscription metadata (from config.toml after /login)
export type KimiProviderMetadata = {
  type: "kimi";
  accountId?: string;
  email?: string;
};
